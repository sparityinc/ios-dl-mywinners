...
- (void) prepareTopView
{
    ...
    [self.amountButton setBackgroundImage:[UIImage imageNamed:@"dollarImgbg.png"] forState:UIControlStateNormal];
    [self.amountButton setBackgroundImage:[UIImage imageNamed:@"dollarEmptyBg"] forState:UIControlStateSelected];
    ...
    [self.amountButton.titleLabel setFont:[UIFont fontWithName:@"Roboto-Light" size:12]];
...
//        [ZLAppDelegate getAppData].currentWager.selectedRaceId = -1;
    ...
    else if(tile.tag == WagerAmount && ([ZLAppDelegate getAppData].currentWager.amount > 0) ){
...
-(void) gotoTracks{
    ...
    UIViewController *viewController = [self.wagerNavigationCoontroller.viewControllers objectAtIndex:WagerTrack];
    [self.wagerNavigationCoontroller popToViewController:viewController animated:NO];
}
...
- (NSString *) titleForIndex:(NSUInteger)index
{
    ...
    if ([self isRunnersSelected] && index == WagerRunners && [[ZLAppDelegate getAppData].currentWager calculateTotalBetAmount] != 0) {
        ...
        if([ZLAppDelegate getAppData].currentWager.amount != 0.0)
            return [NSString stringWithFormat:@"$%.0f", [ZLAppDelegate getAppData].currentWager.amount];
    }
    else {
...
- (UIImage *) backgroundImage:(NSUInteger)index
{
    if ([self isRunnersSelected] && index == WagerRunners && [[ZLAppDelegate getAppData].currentWager calculateTotalBetAmount] != 0) {
        ...
        return [self imageByDrawingTextOnImage:[UIImage imageNamed:@"placewagerbgutton"] withtext:[NSString stringWithFormat:@"$%.02f",[[ZLAppDelegate getAppData].currentWager calculateTotalBetAmount]]];
    }
    else if(index == WagerAmount && [self isAmoutViewLoaded]){
