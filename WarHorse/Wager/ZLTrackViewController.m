...
- (void)viewDidLoad
{
    ...
    self.trackSettingsView.view.frame = CGRectMake(9.0, 25.0, 228.0, 145.0);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if( _race_card.purseUsa == nil || [_race_card.purseUsa length] <= 0){
        ...
        [self.wagerCustomCell.information_Label setText:[NSString stringWithFormat:@"%@ | %@ \nPurse $%@",_race_card.distance != nil ? _race_card.distance : @"", _race_card.trackType != nil ? _race_card.trackType : @"", _race_card.purseUsa != nil ? _race_card.purseUsa : @"N/A"]];
    }
...
- (IBAction)settingsButtonSelected:(id)sender{
    ...
    if(!self.menuShowing){
        ...
        [UIView animateWithDuration:0.3f delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
            self.trackSettingsView.view.frame = CGRectMake(9.0, 42.0, 228.0, 145.0);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [ZLAppDelegate getAppData].currentWager.selectedRaceId = _race_card.currentRaceNumber;
