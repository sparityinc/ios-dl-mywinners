...
@property (strong, nonatomic) IBOutlet UIImageView *ImageView123;
                     ...
                     dispatch_async(dispatch_get_main_queue(), ^{
                       ...
                       //  self.is123Available = [_userInfo ]
                         ...
                         for( ZLRaceDetails * _race_details in _race_card.cardDetails){
                             ...
                             if ([_race_card.is123Available boolValue]) {
                                 
                             }
//                            NSString *eventCodeStr1 = [NSString stringWithFormat:@"%@", _race_card.is123Available];
//                            NSLog(@"eventCodeStr1%@",eventCodeStr1);
//                             
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //NSLog(@"video %@",_race_card.eventCode);
    //
    ...
    if ([[self.openResultsinTable filteredArrayUsingPredicate:valuePredicate] count]!=0) {
        ...
        if( _race_details.purseUsa != nil || [_race_details.purseUsa length] > 0){
            self.raceCustomCell.amountLabel.text = [NSString stringWithFormat:@"Purse $%@",_race_details.purseUsa];
        }
        else{
