...
@property (strong, nonatomic) IBOutlet UILabel *nickNameTitle;
@property (strong, nonatomic) IBOutlet UILabel *nickanameLbl;
...
- (void)viewDidLoad
{
    ...
    if([[WarHorseSingleton sharedInstance] is_nick_name_enabled] == NO){
        [self.currentBalanceView setFrame:CGRectMake(10, 78, 300, 135)];
        [self.accountActivityLabel setFrame:CGRectMake(10, 225, 145, 21)];
        [self.viewBtn setFrame:CGRectMake(235, 223, 75, 25)];
        [self.refreshBtn setFrame:CGRectMake(203, 223, 20, 20)];
        [self.walletTableView setFrame:CGRectMake(0, 255, 320, 265)];
    ...
    [apiWrapper getCurrentBalanceForAccount:nil success:^(NSDictionary *_userInfo)
     {
         ...
         if ([succesMes isEqualToString:@"success"]){
             dispatch_async(dispatch_get_main_queue(), ^{
                 ...
                 NSString *amountStr = [NSString stringWithFormat:@"$%0.2f",([[[_userInfo valueForKey:@"response-content"]valueForKey:@"available_balance"] doubleValue]) / 100.0];
                 
                 NSString *onHoldAmountStr = [NSString stringWithFormat:@"$%0.2f",([[[_userInfo valueForKey:@"response-content"]valueForKey:@"hold_balance"] doubleValue]) / 100.0];
...
-(void)nickNameService{
    ...
    {
        self.nickNameTitle.text = [NSString stringWithFormat:@"%@ :",[[WarHorseSingleton sharedInstance] nick_name_label]];
    ZLAPIWrapper *apiWrapper = [ZLAppDelegate getApiWrapper];
    ...
    [apiWrapper nickNameinfo:nil success:^(NSDictionary *nickInfo){
        ...
        if ([succesMesStr isEqualToString:@"success"]){
            ...
            self.nickanameLbl.text = [NSString stringWithFormat:@"%@",[[nickNmeDic valueForKey:@"response-content"]valueForKey:@"nickName"]];
            self.nickanameLbl.textAlignment = NSTextAlignmentLeft;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.accountActivityArry count]>0){
        ...
        [self.walletCustomCell.cashLabel setBackgroundColor:[UIColor colorWithRed:1.0/255.0f green:142.0/255.0f blue:183.0/255.0f alpha:1.0]];
        ...
        if ([[dic valueForKey:@"betType"] isKindOfClass:[NSString class]]){
            ...
            if ([transactionType isEqualToString:@"Ticket Sell"]){
                //NSLog(@"Ticket Sell");
                creditAmount = [NSString stringWithFormat:@"-$%.02f",[[dic valueForKey:@"debit"] floatValue]];
                ...
                cashStr = @"CNCL";
                [self.walletCustomCell.cashLabel setBackgroundColor:[UIColor colorWithRed:113.0/255.0f green:171.0/255.0f blue:1.0/255.0f alpha:1.0]];
            }else{
            ...
            NSString *betTypeAndRaceStr = [NSString stringWithFormat:@"Race %@ - %@ - $%.02f",[dic valueForKey:@"race"],[dic valueForKey:@"betType"],amt];
            ...
            if ([transactionType isEqualToString:@"Withdrawal"]){
                ...
                [self.walletCustomCell.amountLable setTextColor:[UIColor redColor]];
                cashStr= @"WD";
                [self.walletCustomCell.cashLabel setBackgroundColor:[UIColor colorWithRed:107.0/255.0f green:84.0/255.0f blue:132.0/255.0f alpha:1.0]];
            ...
            else if ([transactionType isEqualToString:@"ACH Deposit "]||[transactionType isEqualToString:@"ACH Deposit Cleared"])
            {
                ...
                [self.walletCustomCell.amountLable setTextColor:[UIColor colorWithRed:60.0/255.0f green:90.0/255.0f blue:0.0/255.0f alpha:1.0]];
                cashStr= @"ACH";
                [self.walletCustomCell.cashLabel setBackgroundColor:[UIColor colorWithRed:237.0/255.0f green:106.0/255.0f blue:62.0/255.0f alpha:1.0]];
            ...
            else if ([transactionType isEqualToString:@"Balance Adjustment"])
            {
                ...
                cashStr= @"ADJ";
                [self.walletCustomCell.cashLabel setBackgroundColor:[UIColor colorWithRed:77.0/255.0f green:99.0/255.0f blue:123.0/255.0f alpha:1.0]];
            }
            ...
            }else if ([transactionType isEqualToString:@"Winner IRS Hold"]||[transactionType isEqualToString:@"Winner IRS Clear"])
            {
                ...
                [self.walletCustomCell.amountLable setTextColor:[UIColor colorWithRed:60.0/255.0f green:90.0/255.0f blue:0.0/255.0f alpha:1.0]];
                cashStr= @"IRS";
                [self.walletCustomCell.cashLabel setBackgroundColor:[UIColor colorWithRed:33.0/256 green:54.0/256 blue:84.0/256 alpha:1.0]];
            }
        ...
        if ([[dic valueForKey:@"meetName"] isKindOfClass:[NSString class]])
        {
            ...
            [self.walletCustomCell.titleLable setText:[dic valueForKey:@"description"]];
