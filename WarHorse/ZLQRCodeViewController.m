...
- (void)getQRcodeImg
{
    ...
    if ([actionStr isEqualToString:@"withdraw"]){
        NSArray *array = [self.amountTF.text componentsSeparatedByString:@"$"];
...
- (IBAction)submitAmount:(id)sender
{
    ...
    if ([self.amountTF.text isEqualToString:@"$"] || [self.amountTF.text isEqualToString:@"$0"] || [self.amountTF.text isEqualToString:@"$0.0"] || [self.amountTF.text isEqualToString:@"$0.00"]) {
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    switch (textField.tag) {
        ...
        case 1: //Amount Text field
        {
            ...
            if ([textField.text isEqualToString:@"$"] && range.length == 1)
            {
                return NO;
            }
            
            if ([textField.text isEqualToString:@"$0"] && range.length == 0 && range.location == 2 && [string intValue] == 0)
            {
                return NO;
            }
