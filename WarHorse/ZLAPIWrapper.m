...
- (id)init
{
    ...
    if (self) {
        ...
        self.baseURL = [NSURL URLWithString:kAPIDevEndpointHost];
        ...
        [resultsMapping addAttributeMappingsFromDictionary:@{
                                                             ...
                                                             @"perf": @"perf",
                                                             @"race": @"race",
                                                        @"is123PayOutsAvailable" :@"is123PayOutsAvailable"
                                                             }];
        ...
        RKObjectMapping *poolMapping = [RKObjectMapping mappingForClass:[ZLPoolPayouts class]];
        [poolMapping addAttributeMappingsFromDictionary:@{
                                                                 @"Amount": @"Amount",
                                                                 @"Position": @"Position",
                                                                 @"Points": @"Points",
                                                                 @"Paid": @"Paid"
        ...
        RKRelationshipMapping * betPoolRelationshipMapping = [RKRelationshipMapping relationshipMappingFromKeyPath:@"PayOuts123" toKeyPath:@"PayOuts123" withMapping:poolMapping];
        [resultsMapping addPropertyMapping:betPoolRelationshipMapping];
        betPoolRelationshipMapping.assignmentPolicy = RKUnionAssignmentPolicy;
        ...
        RKResponseDescriptor *responseDescriptor2 = [RKResponseDescriptor responseDescriptorWithMapping:raceDetailsMapping pathPattern:getCardDetail keyPath:@"response-content.cardDetail.races"
                                                                                            statusCodes:RKStatusCodeIndexSetForClass(RKStatusCodeClassSuccessful)];
        ...
        RKResponseDescriptor *responseDescriptor7 = [RKResponseDescriptor responseDescriptorWithMapping:preLoginResponseContentMapping pathPattern:@"mywinnerscms/services/loadData" keyPath:@"response-content" statusCodes:RKStatusCodeIndexSetForClass(RKStatusCodeClassSuccessful)];
                                                   ...
                                                   success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                                       ...
                                                       if( [responseObject isKindOfClass:[NSDictionary class]]){ //[operation.response statusCode] == 201 &&
                                                           ...
                                                           if( [_status isEqualToString:@"success"]){
                                                               ...
                                                               if (failure) {
                                                                   dispatch_async(dispatch_get_main_queue(), ^{
                                                                       NSError * error = [NSError errorWithDomain:@"MyWinners" code:400 userInfo:_dict];
                                                                       failure(error);
                                                                   });
                                                               }
                                                           }
...
-(void) loadRaceCards:(BOOL) alert
    ...
    if (![[NSUserDefaults standardUserDefaults] valueForKey:@"ClientId"]){
        [[NSUserDefaults standardUserDefaults] setObject:@"MyWinnersCT" forKey:@"ClientId"];
                                ...
                                if(_race_card){
                                    ...
                                    for( ZLRaceDetails * raceDetails in _race_card.cardDetails)
                                    {
                                        for( ZLBetType * betType in raceDetails.betTypes){
                                            if( [betType.name isEqualToString:@"WIN" ] || [betType.name isEqualToString:@"PLC" ] || [betType.name isEqualToString:@"SHW" ] || [betType.name isEqualToString:@"WP" ] ||
...
//-(void) getRaceResultsByMeet:(int) meet WithPerf:(int) perf
//                     success:(void (^)(NSDictionary* _userInfo))success
//                     failure:(void (^)(NSError *error))failure{
//    [self headersLoading];
//    
...
//    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
//    [formatter setDateFormat:@"yyyy/MM/dd"];
//    NSMutableDictionary * jsonDict = [NSMutableDictionary dictionaryWithDictionary:@{@"meet":[NSString stringWithFormat:@"%d",meet],@"perf":[NSString stringWithFormat:@"%d",perf]}];
//    //NSLog(@"jsonDict %@",jsonDict);
//    
...
- (void)loadMyBetsDataFromDate:(NSString *)_fromDateStr toDate:(NSString *)_toDateStr success:(void (^)(NSDictionary *))success failure:(void (^)(NSError *))failure
{
    ...
    [[RKObjectManager sharedManager] setRequestSerializationMIMEType:RKMIMETypeJSON];
    [[[RKObjectManager sharedManager] HTTPClient] setParameterEncoding:AFJSONParameterEncoding];
    [[[RKObjectManager sharedManager] HTTPClient] postPath:myBets2
                                                parameters:jsonDict
                                                   success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                                       if([operation.response statusCode] == 201 && [responseObject isKindOfClass:[NSDictionary class]]){
                                                           NSDictionary* _dict = (NSDictionary*)responseObject;
                                                           //NSLog(@"Mybets Dict %@",_dict);
...
- (void)preloginBanners:(NSDictionary *)params success:(void (^)(NSDictionary* _userInfo))success failure:(void (^)(NSError *error))failure
{
    ...
    [[RKObjectManager sharedManager] setRequestSerializationMIMEType:RKMIMETypeJSON];
    [[[RKObjectManager sharedManager] HTTPClient] setParameterEncoding:AFJSONParameterEncoding];
    [[[RKObjectManager sharedManager] HTTPClient] postPath:@"mywinnerscms/services/loadData"
                                                parameters:params
                                                   success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                                       if([operation.response statusCode] == 201 && [responseObject isKindOfClass:[NSDictionary class]]){
                                                           //NSString *succesMes = [responseObject valueForKey:@"response-status"];
...
- (void)userDigitalVoucher:(NSDictionary *)dvDeatails success:(void (^)(NSDictionary* _userInfo))success failure:(void (^)(NSError *error))failure{
    ...
    [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"client_id" value:@"MyWinnersCT-ODP"];
    [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"user_pin" value:@""];
    [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"account_id" value:@""];
    [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"user_id" value:@""];
    [[RKObjectManager sharedManager].HTTPClient setDefaultHeader:@"user_password" value:@""];
...
//nickname
- (void)nickNameinfo:(NSDictionary *) _params success:(void (^)(NSDictionary* nickInfo))success failure:(void (^)(NSError *error))failure{
    [self headersLoading];
    
    
    
    [[[RKObjectManager sharedManager] HTTPClient] getPath:nickNameInfo parameters:nil
                                                  success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                                      //NSLog(@"responseObject %@",responseObject);
