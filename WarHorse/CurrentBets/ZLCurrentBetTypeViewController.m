...
- (IBAction)refresh_Clicked:(id)sender
{
    ...
    [self.mtpControl setMTPSelectedForPage:1];
    [self loadDataForDate:self.selectedDate withHud:YES];
...
- (void)prepareViewForFilteredTracks:(NSMutableArray *)myTransactionTracks
{
    ...
    for (int i = 0; i <= [myTransactionTracks count]/5; i++) {
     
        [self.backgroundImagesArray addObject:[UIImage imageNamed:@"violet.png"]];
        [self.backgroundImagesArray addObject:[UIImage imageNamed:@"blue.png"]];
        [self.backgroundImagesArray addObject:[UIImage imageNamed:@"orange.png"]];
        [self.backgroundImagesArray addObject:[UIImage imageNamed:@"green.png"]];
        [self.backgroundImagesArray addObject:[UIImage imageNamed:@"red.png"]];
    }
    ...
    for( NSMutableDictionary *transactionTrack in myTransactionTracks){
        ...
        for (NSMutableDictionary *bets in [transactionTrack mutableArrayValueForKey:@"bets"]) {
            if( [[bets valueForKey:@"betAmount"] intValue] != 0 ){
                ...
                self.totalBets += [[bets valueForKey:@"debit"] integerValue] / [[bets valueForKey:@"betAmount"] integerValue];
            }
            else{
                self.totalBets = 0;
            }
        ...
        [raceNumLabel setText:[NSString stringWithFormat:@"Race: %@", [transactionTrack valueForKey:@"raceNumber"]]];
        [raceNumLabel setFont:[UIFont fontWithName:@"Roboto-Light" size:13]];
        ...
        if( [[transactionTrack valueForKey:@"MTP"] intValue] != 999 && [[transactionTrack valueForKey:@"MTP"] intValue] >= 0 )
            [self.mtp_Label setText:[NSString stringWithFormat:@"%@",[transactionTrack valueForKey:@"MTP"]]];
        else
        ...
        self.totalBetLabel=[[UILabel alloc]initWithFrame:CGRectMake(8, raceNumLabel.frame.size.height+25, 100, LabelHeight)];
        ...
        if( self.inPlay){
            ...
            [self.resultLabel setText:[NSString stringWithFormat:@"$%0.2f",self.totalAmount]];
            }
        [self.resultLabel setFont:[UIFont fontWithName:@"Roboto-Light" size:13]];
        [containerImageView addSubview:self.resultLabel];
        
        
        self.amountLabel=[[UILabel alloc]initWithFrame:CGRectMake( 116,raceNumLabel.frame.size.height+25, 100, LabelHeight)];
        ...
        if( self.inPlay){
            ...
            self.amountLabel.frame = CGRectMake(118,raceNumLabel.frame.size.height+25, 100, LabelHeight);
        ...
        if( self.inPlay){
            [self.dollarLabel setText:[NSString stringWithFormat:@"$%.2f",self.totalAmount]];
        }
        else{
            [self.dollarLabel setText:[NSString stringWithFormat:@"$%.2f",(winnings/100.0)]];
        }
...
-(void) recalculateAmountAndUpdateLabel {
    ...
    for( NSMutableArray * myBetTransactions in [self.selectedRaceDetail valueForKey:@"bets"]){
       // NSLog(@"bet type %@",[myBetTransactions valueForKey:@"selection"]);
        if( [[myBetTransactions valueForKey:@"selection"] rangeOfString:@"Cancelled"].location == NSNotFound){
            totalBets += [[myBetTransactions valueForKey:@"debit"] doubleValue] / [[myBetTransactions valueForKey:@"betAmount"] doubleValue];
    ...
    if ([self.dollarLabels count]) {
        ...
        [lblDollar setText:[NSString stringWithFormat:@"$%.2f",totalAmount]];
    }
...
- (void)selectRace:(int) index{
    ...
    if( raceDetail != nil){
        ...
        for (int i = 0; i < [[raceDetail mutableArrayValueForKey:@"bets"] count]; i++) {
            ...
            //NSLog(@"cancel %@",[myBetTransactions valueForKey:@"selection"]);
            
            if(!([[myBetTransactions valueForKey:@"selection"] rangeOfString:@"Cancelled"].location == NSNotFound)){
                
                //NSLog(@"myBetTransactions %@",myBetTransactions);
                //[self.selectedRaceDetail mutableArrayValueForKey:@"bets" ;
...
- (void)loadDataForDate:(NSDate *)date withHud:(BOOL) hud
{
    ...
    [apiWrapper loadMyBetsDataFromDate:fromDateStr toDate:toDateStr
...
-(IBAction)video_Clicked:(id)sender
{
    ...
    */
    ...
    NSDictionary *argumentsDic = @{@"client_id": @"MyWinnersCT",
                                   @"videoType": @"live",
                                   @"format": @"stream",
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [currentBetCustomCell.amountDollar_Label setFrame:CGRectMake(220, 11, 60, 21)];
    ...
    currentBetCustomCell.betDollar_Label.text = [NSString stringWithFormat:@"$%.2f",[[_transaction valueForKey:@"betAmount"] floatValue]];
    ...
    currentBetCustomCell.amountDollar_Label.text = [NSString stringWithFormat:@"$%.2f",[[_transaction valueForKey:@"debit"] floatValue]];
    ...
    if( !self.inPlay || [[_transaction valueForKey:@"selection"] rangeOfString:@"Cancelled"].location != NSNotFound){
        if(!self.inPlay){
            ...
            [currentBetCustomCell.betType_Label setFrame:CGRectMake(70, 11, 165, 21)];
            ...
            if([[_transaction valueForKey:@"result"] isEqualToString:@"WON"]){
                ...
                currentBetCustomCell.amountDollar_Label.text = [NSString stringWithFormat:@"$%.2f",[[_transaction valueForKey:@"winningAmount"] floatValue]/100.0];
