...
- (IBAction)submitButton:(id)sender
{
    ...
    [apiWrapper withdrawalRequest:checkDetailsDict success:^(NSDictionary *_userInfo) {
        ...
        if ([[_userInfo valueForKey:@"response-status"] isEqualToString:@"success"]) {
            responseMes =  [NSString stringWithFormat:@"Your withdrawal was successful and your current available balance is $%@",[[_userInfo valueForKey:@"response-content"] valueForKey:@"availableBalance"]];
