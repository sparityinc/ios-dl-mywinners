...
- (void)viewDidLoad
{
    ...
    [self.label1 setText:@"$1 = 400 Points redemption\n(Minimum amount for conversion is $10) "];
...
- (IBAction)submitBtnClicked:(id)sender
{
    ...
    if ([self.rewardsAvailableBalance intValue] > (pointsInt * 400)) {
        ...
        if (pointsInt<10){
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Information!" message:@"Minimum amount for converstion is $10" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
            [alert show];
            return;
        }
...
//                 NSString *mes = [NSString stringWithFormat:@"$%@.00 worth %d points shall be credited to your account.",self.amountTxtFld.text,pointsInt];
                 NSString *mes = [NSString stringWithFormat:@"Credited %d points of $%@.00 worth credited to your account",pointsInt,self.amountTxtFld.text];
