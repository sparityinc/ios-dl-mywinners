...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([[[self.paymentChargesDetailsDict valueForKey:@"response-content"] valueForKey:@"statusMessage"]isEqualToString:@"SUCCESS"]){
        ...
        self.flat_TF.text = [NSString stringWithFormat:@"$%.2f",self.processingFlatFee];
    ...
    self.creditCardDiscreptionLbl.text = [NSString stringWithFormat:cardMaxandMinAmount,self.creditCardMin,self.creditCardMax,self.processFeeStr,@"%"];
...
- (IBAction)addAmountClicked:(id)sender
{
    ...
    self.totalAmountStr = [self.totalAmountStr stringByReplacingOccurrencesOfString:@"$" withString:@""];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    */
    ...
    self.processingFeeStr = [self.processingFeeStr stringByReplacingOccurrencesOfString:@"$" withString:@""];
    ...
    self.totalAmountStr = [NSString stringWithFormat:@"$%0.2f",finalAmount];
