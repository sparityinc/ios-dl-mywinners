...
- (void)viewDidLoad
{
    ...
    UILabel *scrChangesTitle = [[UILabel alloc]initWithFrame:CGRectMake(44, 36, 300, 19)];
    [scrChangesTitle setBackgroundColor:[UIColor clearColor]];
    [scrChangesTitle setTextColor:[UIColor whiteColor]];
    [scrChangesTitle setText:@"Scratches & Changes/Corrections"];
    [self.headerImageView addSubview:scrChangesTitle];
    
    if (isPostLoginFlag == YES){
        [scrChangesTitle setFont:[UIFont fontWithName:@"Roboto-Light" size:13]];
        ...
        [scrChangesTitle setFont:[UIFont fontWithName:@"Roboto-Light" size:15]];

        [self.homeBtn setBackgroundImage:[UIImage imageNamed:@"homebutton.png"] forState:UIControlStateNormal];
...
- (void)loadDataForSelectedDate:(NSDate *)selectedDate
{
    ...
    [preloginapiwrapper loadSCRChangesDataForSelectedDate:nil success:^(NSDictionary *_userInfo) {
        ...
        if ([[[_userInfo valueForKey:@"Deposit_Status"] valueForKey:@"response-content"] count]) {
            ...
            for (int i = 0; i <= [self.scrChangesDetailObjectsArray count]/5; i++) {
